...
class FasterRCNN(nn.Module):
    ...
    @nograd
    def predict(self, imgs, sizes=None, visualize=False, use_preset=True, nms=0.2, score=0.05):
        ...
        self.eval()
        if visualize:
            if use_preset: self.use_preset('visualize')
            prepared_imgs = list()
            sizes = list()
            for img in imgs:
                size = img.shape[1:]
        ...
        if use_preset: self.use_preset('evaluate')
        self.train()
